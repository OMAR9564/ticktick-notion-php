...
class IntegrationController extends BaseController
{
    ...
    {
        // Similar to handleCallback but can have additional logic
        // You can use this for different handling, such as logging or another OAuth integration.
        $code = $this->request->getGet('code');
        if (!$code) {
            session()->setFlashdata('sync_error', 'Authorization failed.');
            return redirect()->to('/integration');
    ...
    private function getAccessToken($code)
    {
        ...
        $response = $client->request('POST', 'https://ticktick.com/oauth/token', [
            ...
            'verify' => false // SSL doğrulamasını kapatır
        ]);
    ...
    private function getTickTickTasks($accessToken)
    {
        try {
            $client = new Client();
            $response = $client->request('GET', 'https://api.ticktick.com/open/v1/task', [
                'headers' => [
